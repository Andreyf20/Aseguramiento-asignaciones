...
# Enero, Marzo, Mayo, Julio, Agosto, Octubre y Diciembre meses con 31 días
dias31 = [1, 3, 5, 7, 8, 10, 12]
# Abril, Junio, Septiembre y Noviembre meses con 30 días
dias30 = [4, 6, 9, 11]
...
def fecha_es_tupla(p_date: tuple) -> bool:
    ...
    # Se validan que sean 3 números enteros
    if((not isinstance(p_date[0], int)) or (not isinstance(p_date[1], int)) or (not isinstance(p_date[2], int))):
        return False
...
def bisiesto(p_year: int) -> bool:
    ...
    # Si el año es divisible entre 100 no es bisisesto a menos que también sea divisibles entre 400
    if((p_year % 100) == 0 and (p_year % 400) != 0):
        return False
...
def fecha_es_valida(p_date: tuple) -> bool:
    ...
    # Validar si es uno de los 12 meses permitidos, y si el día es mayor a cero
    if((p_date[1] < 1 or p_date[1] > 12) or p_date[2] < 1):
        return False
...
def dia_siguiente(p_date: tuple) -> tuple:
    ...
    if(not fecha_es_tupla(p_date)):
        raise Exception(
            'La fecha ingresada no es válida: El formato es incorrecto o se tienen número no positivo, no enteros')

    if(not fecha_es_valida(p_date)):
        raise Exception(
            'La fecha ingresada no es válida: La fecha no es parte del calendario gregoriano')

    # Se le agrega uno al día y se valida
    p_date_var = (p_date[0], p_date[1], p_date[2] + 1)
    if(fecha_es_valida(p_date_var)):
        return p_date_var

    # Si sumandole uno al día es inválido entonces se pasa al siguiente mes
    p_date_var = (p_date_var[0], p_date_var[1] + 1, 1)
    if(fecha_es_valida(p_date_var)):
        return p_date_var

    # Si sumandole uno al mes es inválido entonces se pasa al siguiente año
    return (p_date_var[0] + 1, 1, 1)
...
def edad_hoy(p_date: tuple) -> tuple:
    ...
    if(not pastDay):
        ...
        # Encuentro la cantidad de días cumplidos
        p_date = (today[0], p_date[1], p_date[2])
...
def dia_semana(p_date: tuple) -> int:
    ...
    # El número del mes
    m = p_date[1] - 2 if p_date[1] > 2 else (
        11 if p_date[1] == 1 else (12 if p_date[1] == 2 else -1))
    if(m == -1):
...
calendar = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto',
            'Setiembre', 'Octubre', 'Noviembre', 'Diciembre']  # lista con los meses del año

# lista con las iniciales de la semana
week = ['D', 'L', 'K', 'M', 'J', 'V', 'S']
...
def imprimir_3x4(year: int) -> None:
    ...
    if year < 1583:
        raise Exception(
            'El año ingresado no es válido: El año no es parte del calendario gregoriano')
    else:

        # encontrar día en que se inicia año solicitado, se usa función de R7
        ...
        while(cont < 12):
            ...
            if cont == 3 or cont == 7 or cont == 11:  # comenzar una fila de meses cada 4 columnas
                ...
                while(i < 4):
                    print('| ', end='')
                    # imprimir iniciales de la semana 4 veces
                    print(''.join(['{0:<3}'.format(w) for w in week]), end='')
                    i = i+1
                print('\n| ', end='')
                ...
                while(day <= 40):
                    if day >= 29:
                        if leap == True and month == 2 and day == 29:
                            # formato de impresión dejando 3 espacios entre números y letras
                            print('{0:<3}'.format(29), end='')
...
def fecha_futura(p_date: tuple, days: int) -> tuple:
    ...
    if(days < 0 or not isinstance(days, int)):
        raise Exception(
            'El valor de días ingresado debe ser no negativo entero')
...
if __name__ == "__main__" and tests_enabled:
    ...
    # Fecha mayor o menor
    testCase.assertRaises(Exception, fecha_despues,
                          (1400, 12, "hola"), (1400, 12, 75))
    testCase.assertRaises(Exception, fecha_despues,
                          (1400, 13, 24), (1400, 12, 75))
